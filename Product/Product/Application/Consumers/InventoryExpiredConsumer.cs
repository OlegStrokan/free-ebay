...
public sealed class InventoryExpiredConsumer(
    ISender sender,
    ILogger<InventoryExpiredConsumer> logger) : IInventoryEventConsumer
{
        ...
        foreach (var item in @event.Items)
        {
            ...
            if (!result.IsSuccess)
                ...
                logger.LogInformation(
                    "Stock restored by {Qty} for product {ProductId} (reservation {ReservationId} expired)",
