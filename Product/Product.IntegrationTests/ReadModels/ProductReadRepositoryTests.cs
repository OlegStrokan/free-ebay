...
public sealed class ProductReadRepositoryTests : IClassFixture<IntegrationFixture>
{
    ...
    // Persists a product directly, bypassing the persistence service (no outbox)
    private static async Task<Domain.Entities.Product> SeedProductAsync(
        ProductDbContext db,
        Guid categoryId,
        Guid? sellerId = null,
    ...
    public async Task GetByIdAsync_ShouldReturnProductDetailDto_WithCategoryName()
    {
        ...
        var dto = await repo.GetByIdAsync(product.Id.Value);

        dto.Should().NotBeNull();
        dto!.ProductId.Should().Be(product.Id.Value);
    ...
    public async Task GetByIdsAsync_ShouldReturnMatchingProductDetailDtos()
    {
        ...
        var results = await repo.GetByIdsAsync([p1.Id.Value, p2.Id.Value, unrelatedId]);
    ...
    public async Task GetPricesByIdsAsync_ShouldReturnPriceDtos_ForExistingProducts()
    {
        ...
        var prices = await repo.GetPricesByIdsAsync([p1.Id.Value, p2.Id.Value]);

        prices.Should().HaveCount(2);
        prices.Should().Contain(p => p.ProductId == p1.Id.Value && p.Price == 19.99m);
        prices.Should().Contain(p => p.ProductId == p2.Id.Value && p.Price == 99.00m);
    }
