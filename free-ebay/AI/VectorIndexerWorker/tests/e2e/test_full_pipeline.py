...
async def test_update_event_overwrites_existing_product(indexer: Indexer, qdrant_url_e2e: str) -> None:
    ...
    update_payload = {**_PRODUCT, "name": "Updated Keyboard", "stock_quality": 0}
...
@pytest.fixture(autouse=True)
async def fresh_collection(qdrant_url_e2e: str):
    async with AsyncQdrantClient(url=qdrant_url_e2e) as raw:
        collections = await raw.get_collections()
...
async def indexer(qdrant_url_e2e: str, embedding_mock) -> Indexer:
    embedding_client = EmbeddingClient()
    qdrant_client = QdrantIndexClient.__new__(QdrantIndexClient)
    qdrant_client._client = AsyncQdrantClient(url=qdrant_url_e2e)

    with patch("qdrant_client.settings") as mock_settings:
        mock_settings.qdrant_collection = COLLECTION
        mock_settings.vector_dimensions = 4
        await qdrant_client.ensure_collection()
...
def _make_msg(event_type: str, payload: dict):
...
async def test_create_event_indexes_product_in_qdrant(indexer: Indexer, qdrant_url_e2e: str) -> None:
    ...
    async with AsyncQdrantClient(url=qdrant_url_e2e) as raw:
        result = await raw.retrieve(collection_name=COLLECTION, ids=["e2e-prod-1"], with_payload=True)
...
async def test_update_event_overwrites_existing_product(indexer: Indexer, qdrant_url_e2e: str) -> None:
    ...
    assert len(result) == 1
    assert result[0].payload["name"] == "Updated Keyboard"
    assert result[0].payload["status"] == "out_of_stock"
...
async def test_delete_event_removes_product_from_qdrant(indexer: Indexer, qdrant_url_e2e: str) -> None:
    ...
    async with AsyncQdrantClient(url=qdrant_url_e2e) as raw:
        result = await raw.retrieve(collection_name=COLLECTION, ids=["e2e-prod-1"])
