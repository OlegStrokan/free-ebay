...
public class GetPaymentByOrderIdQueryHandlerTests
{
    ...
    private GetPaymentByOrderIdQueryHandler BuildHandler() => new(_paymentRepository);

    [Fact]
    public async Task Handle_ShouldReturnFailure_WhenPaymentNotFound()
    {
        _paymentRepository.GetByOrderIdAndIdempotencyKeyAsync(
                Arg.Any<string>(),
                Arg.Any<IdempotencyKey>(),
                Arg.Any<CancellationToken>())
            .Returns((Payment?)null);

        var result = await BuildHandler().Handle(new GetPaymentByOrderIdQuery("order-1", "idem-1"), CancellationToken.None);
